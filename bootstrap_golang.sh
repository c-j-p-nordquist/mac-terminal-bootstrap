...
install_tool() {
    echo "Installing $1..."
    if [[ $1 == "iterm2" || $1 == "visual-studio-code" || $1 == "goland" ]]; then
        brew install --cask $1
    else
        brew install $1
    fi
...
tools=(
    ...
    "mockgen:Mock code generator for Go"
...
# Special handling for Go-specific setup
if [[ " ${choices[*]} " == *" go "* ]]; then
    # Set GOPATH and add to PATH
    echo 'export GOPATH=$HOME/go' >> ~/.zshrc
    echo 'export PATH=$PATH:$GOPATH/bin' >> ~/.zshrc
    
    # Install some common Go tools
...
if [[ " ${choices[*]} " == *" zsh "* ]]; then
    ...
    sed -i '' 's/plugins=(git)/plugins=(git golang zsh-syntax-highlighting zsh-autosuggestions)/g' ~/.zshrc
fi
...
if [[ " ${choices[*]} " == *" go "* ]]; then
    echo "Go has been installed. Remember to set up your workspace and GOPATH if needed."
fi